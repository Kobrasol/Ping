...
objStr1 = "<TABLE border='1'>" & vbCrLf & "<TR>" & vbCrLf & "<TH><input type='CheckBox' name='CheckboxOption' id='pin0' onClick='ForEach'></TH>" & vbCrLf & "<TH><FONT SIZE='' COLOR='#0000FF'>Узел:</FONT></TH>" & vbCrLf & "<TH><FONT SIZE='' COLOR='#0000FF'>IP Adress:</FONT></TH>" & vbCrLf & "</TR>" & vbCrLf
objStr3 = "</TABLE>"
	...
	objStr2 = "<TR>"& vbCrLf &"<TD><input type='CheckBox' name='CheckboxOption' id='pin" & n+1 & "' " & flag & "></TD>"& vbCrLf &"<TD><input name='t" & n+1 &"' size='15' value='" & text1 & "'></TD>" & vbCrLf &"<TD><input name='CompName"& n+1 &"' size='15' value='" & text2 & "'></TD>"& vbCrLf &"</TR> "& vbCrLf 
...
sub XMLsave_onclick()
	...
	boolpin = document.getElementByID("pin" & i).checked 
	If boolpin Then objTrue = "True|" else objTrue = "False|"
	objText = objTrue & document.getElementByID("t" & i).value & "|" & document.getElementByID("CompName" & i).value
